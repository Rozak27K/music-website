...
<section class="bg-[#f4f4f5] py-12">
    <div class="mx-auto max-w-4xl px-6">
        ...
        <form action="{{ route('admin.galeri.store') }}" method="POST" enctype="multipart/form-data" class="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
            @csrf
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4" class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 font-semibold outline-none focus:border-purple-500 focus:bg-white focus:ring-4 focus:ring-purple-100">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="mt-2 text-sm font-semibold text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="kategori" name="kategori" class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 font-semibold outline-none focus:border-purple-500 focus:bg-white focus:ring-4 focus:ring-purple-100">
                        @foreach (['Dokumentasi', 'Band', 'Event', 'Paduan Suara'] as $kategori)
                            <option value="{{ $kategori }}" @selected(old('kategori', 'Dokumentasi') === $kategori)>
                                {{ $kategori }}
                            </option>
                        @endforeach
                    </select>
