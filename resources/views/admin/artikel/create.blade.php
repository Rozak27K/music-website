...
<section class="bg-[#f4f4f5] py-12">
    <div class="mx-auto max-w-4xl px-6">
        ...
        <form action="{{ route('admin.artikel.store') }}" method="POST" enctype="multipart/form-data" class="rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
            @csrf

            <div class="space-y-5">
                <div>
                    <label for="judul" class="mb-2 block text-sm font-black text-[#202427]">Judul</label>
                    <input type="text" id="judul" name="judul" value="{{ old('judul') }}" class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 font-semibold outline-none focus:border-purple-500 focus:bg-white focus:ring-4 focus:ring-purple-100">
                    @error('judul')
                ...
                <div>
                    ...
                    <textarea id="ringkasan" name="ringkasan" rows="3" class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 font-semibold outline-none focus:border-purple-500 focus:bg-white focus:ring-4 focus:ring-purple-100">{{ old('ringkasan') }}</textarea>
                    @error('ringkasan')
                        <p class="mt-2 text-sm font-semibold text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="isi" name="isi" rows="8" class="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 font-semibold outline-none focus:border-purple-500 focus:bg-white focus:ring-4 focus:ring-purple-100">{{ old('isi') }}</textarea>
                    @error('isi')
                        <p class="mt-2 text-sm font-semibold text-red-600">{{ $message }}</p>
                    @enderror
                </div>
